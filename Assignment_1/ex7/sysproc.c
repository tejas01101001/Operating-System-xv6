...
int sys_wolfie(void)
{
  ...
  // Fetch the 1st 32 bit call argument and assign it to the size of the buffer
  // Also Check that the pointer to variable size's address lies within the process address space if it does not then return -1.
  if (argint(1, &size) == -1)
  {
    return -1;
  }
  ...
  //copy the wolfie picture to the buffer and return the size of the picture
